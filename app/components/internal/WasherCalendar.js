...
var WasherCalendar = React.createClass({
  ...
  getAllAppointments: function(evt) {
    $.ajax({
                url: useUrl + '/acuity',
                dataType: 'json',
                cache: false,
  ...
  render: function(){
    ...
    //   })
    // });
    var sortedCompanies = companyData.sort(
      function(a, b) {
    ...
    var appointments_DB = sortedCompanies.map(function(item){
      if(item.company == "DayBreakGames"){
        return (
          <ul style={{borderColor:"white", borderStyle:"solid", borderWidth: "0 0 1px 0"}}>
          <li> <strong> {item.firstName + " " + item.lastName + ":"} </strong>
              <div> {item.type} </div>
              <div> {item.phone} </div>
              <div> {item.time + "-" +  item.endTime} </div>
            </li>
          </ul>
        );
    ...
    var appointments_A = sortedCompanies.map(function(item){
      if(item.company == "Alexandria"){
        return (
          <ul> <li> <strong> {item.firstName + " " + item.lastName + ":"} </strong>
              <div> {item.type} </div>
              <div> {item.phone} </div>
              <div> {item.time + "-"  + item.endTime} </div>
            </li>
    ...
    return(
      ...
      Appointments:
      <div >
      <div style={{backgroundColor:"#E08989", fontSize:"25px", width:"1000px"}}> <strong > Alexandria </strong>
      </div> - {appointments_A} </div>

      <div style={{backgroundColor:"#94AAC6"}}> <strong> DayBreakGames </strong>: {appointments_DB} </div>

      {JSON.stringify(companyData, null, 4)}
      <div style={{height:"200x", width:"100%", backgroundColor:"red"}}> </div>
      {JSON.stringify(sortedCompanies, null, 4)}
      </div>
    )
  }
});
